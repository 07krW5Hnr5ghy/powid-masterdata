...
@Repository
public class KardexOperationTypeRepositoryCustomImpl implements KardexOperationTypeRepositoryCustom {
    @PersistenceContext(name = "entityManager")
    private EntityManager entityManager;
    @Override
    public Page<KardexOperationType> searchForKardexOperationType(String name, OffsetDateTime registrationStartDate, OffsetDateTime registrationEndDate, OffsetDateTime updateStartDate, OffsetDateTime updateEndDate, String sort, String sortColumn, Integer pageNumber, Integer pageSize, Boolean status) {
        CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
        CriteriaQuery<KardexOperationType> criteriaQuery = criteriaBuilder.createQuery(KardexOperationType.class);
        Root<KardexOperationType> itemRoot = criteriaQuery.from(KardexOperationType.class);
        ...
        if (!StringUtils.isBlank(sort) && !StringUtils.isBlank(sortColumn)) {
            List<Order> kardexOperationTypeList = new ArrayList<>();
            if (sort.equalsIgnoreCase("ASC")) {
                kardexOperationTypeList = listASC(sortColumn, criteriaBuilder, itemRoot);
            }
            if (sort.equalsIgnoreCase("DESC")) {
                kardexOperationTypeList = listDESC(sortColumn, criteriaBuilder, itemRoot);
            }
            criteriaQuery.where(conditions.toArray(new Predicate[] {})).orderBy(kardexOperationTypeList);
        } else {
            criteriaQuery.where(conditions.toArray(new Predicate[] {}));
        }

        TypedQuery<KardexOperationType> orderTypedQuery = entityManager.createQuery(criteriaQuery);
    ...
    public List<Predicate> predicateConditions(
            ...
            Root<KardexOperationType> itemRoot) {
