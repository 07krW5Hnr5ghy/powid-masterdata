...
@RestController
@CrossOrigin({ "*" })
@RequestMapping("kardex-operation-type")
@AllArgsConstructor
public class KardexOperationTypeController {
    ...
    public ResponseEntity<ResponseSuccess> save(
        ...
        CompletableFuture<ResponseSuccess> result = iKardexOperationType.save(name, tokenUser);
    ...
    public ResponseEntity<ResponseDelete> delete(
            @RequestParam("name") String name,
            @RequestParam("tokenUser") String tokenUser) throws BadRequestExceptions, ExecutionException, InterruptedException {
        CompletableFuture<ResponseDelete> result = iKardexOperationType.delete(name, tokenUser);
        return new ResponseEntity<>(result.get(), HttpStatus.OK);
    }
    @GetMapping()
    //@PreAuthorize("hasAnyAuthority('ROLE:ADMINISTRATION','ROLE:MARKETING','ROLE:STOCK') and hasAuthority('ACCESS:COLOR_GET')")
    public ResponseEntity<List<KardexOperationTypeDTO>> listColor(
            @RequestParam("user") String user
    ) throws BadRequestExceptions, ExecutionException, InterruptedException {
        CompletableFuture<List<KardexOperationTypeDTO>> result = iKardexOperationType.listKardexOperationType(user);
        return new ResponseEntity<>(result.get(), HttpStatus.OK);
    }
    @GetMapping(value = "list")
    //@PreAuthorize("hasAnyAuthority('ROLE:ADMINISTRATION','ROLE:MARKETING','ROLE:STOCK') and hasAuthority('ACCESS:COLOR_GET')")
    public ResponseEntity<Page<KardexOperationTypeDTO>> list(
        ...
        CompletableFuture<Page<KardexOperationTypeDTO>> result = iKardexOperationType.list(
                name,
    ...
    public ResponseEntity<ResponseSuccess> activate(
        ...
        CompletableFuture<ResponseSuccess> result = iKardexOperationType.activate(name,tokenUser);
        return new ResponseEntity<>(result.get(),HttpStatus.OK);
    }
    @GetMapping("filter")
    //@PreAuthorize("hasAnyAuthority('ROLE:ADMINISTRATION','ROLE:MARKETING','ROLE:STOCK') and hasAuthority('ACCESS:COLOR_GET')")
    public ResponseEntity<List<KardexOperationTypeDTO>> listFilter(
            @RequestParam("user") String user
    ) throws BadRequestExceptions, ExecutionException, InterruptedException {
        CompletableFuture<List<KardexOperationTypeDTO>> result = iKardexOperationType.listFilter(user);
        return new ResponseEntity<>(result.get(), HttpStatus.OK);
    }
}
